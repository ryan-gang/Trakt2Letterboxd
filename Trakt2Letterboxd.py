...
class TraktImporter(object):
    ...
    def __init__(self):
        ...
        self.api_clid = (
            "b04da548cc9df60510eac7ec1845ab98cebd8008a9978804a981bff7e73ab270"
        )
        self.api_clsc = (
            "a880315fba01a5e5f0ad7de12b7872e36826a9359b2f419122a24dee1b2cb600"
        )
        self.api_token = None
    ...
    def authenticate(self):
        ...
        got_token = self.__poll_for_auth(
            dev_code_details["device_code"],
    ...
    def __poll_for_auth(self, device_code, interval, expiry):
        ...
        data = """{{ "code":          "{0}",
                     "client_id":     "{1}",
                     "client_secret": "{2}" }}
                       """.format(device_code, self.api_clid, self.api_clsc).encode(
            "utf8"
        )
    ...
    def get_movie_list(self, list_name):
        ...
        while page <= page_limit:
            request = Request(
                ...
                + "/sync/"
                + list_name
                + "/movies?page={0}&limit=10".format(page),
                headers=headers,
            )
            try:
                ...
                if response_body:
                    extracted_movies.extend(
                        self.__extract_fields(json.loads(response_body), ratings)
                    )
            except HTTPError as err:
    ...
    def __get_rating(ratings, ids):
        for rating in ratings:
            if (
                ids["imdb"] == rating["imdb"]
                or ids["trakt"] == rating["trakt"]
                or ids["tmdb"] == rating["tmdb"]
                or ids["slug"] == rating["slug"]
            ):
